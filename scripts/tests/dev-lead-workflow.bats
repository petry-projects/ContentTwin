...
#     rate), or
#   • letting `uses:`/`agent_ref` fall off the moving `dev-lead/v139-stable` channel
...
@test "dev-lead uses: pins the org reusable at the dev-lead/v139-stable channel (not @main or a SHA)" {
  run python3 -c "
import sys, yaml
wf = yaml.safe_load(open(sys.argv[1])) or {}
job = (wf.get('jobs') or {}).get('dev-lead') or {}
uses = job.get('uses', '')
expected = 'petry-projects/.github-private/.github/workflows/dev-lead-reusable.yml@dev-lead/v139-stable'
assert uses == expected, f'uses: must pin the dev-lead/v139-stable channel, got: {uses!r}'
print('ok')
" "$WORKFLOW"
  [ "$status" -eq 0 ]
  [[ "$output" == "ok" ]]
...
@test "dev-lead agent_ref matches the uses: channel (both dev-lead/v139-stable)" {
  run python3 -c "
import sys, yaml
wf = yaml.safe_load(open(sys.argv[1])) or {}
job = (wf.get('jobs') or {}).get('dev-lead') or {}
agent_ref = (job.get('with') or {}).get('agent_ref', '')
assert agent_ref == 'dev-lead/v139-stable', f'agent_ref must be dev-lead/v139-stable, got: {agent_ref!r}'
uses = job.get('uses', '')
